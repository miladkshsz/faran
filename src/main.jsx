...
function App(){
 ...
 const exportNoInvoice=async()=>{if(!niPages.length)return;setNiBusy(true);try{const pdf=new jsPDF({orientation:"portrait",unit:"mm",format:"a4",compress:true});for(let i=0;i<niPages.length;i++){const c=await makePageCanvas(document.getElementById(`ni-${i}`));if(i)pdf.addPage("a4","portrait");pdf.addImage(c.toDataURL("image/jpeg",.99),"JPEG",0,0,210,297,undefined,"FAST");}pdf.save(`فرم-بدون-فاکتور-${header.date||"بدون-تاریخ"}.pdf`);}finally{setNiBusy(false)}};
